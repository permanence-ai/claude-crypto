...
TEST_F(AeadTests, AesGcmEncryptProducesOutput) {
    const auto r = run_scli(scli(),
        std::string("aead --algo aes256-gcm --op encrypt --key ") + kKey + " --input " + kPlain);
...
TEST_F(AeadTests, AesGcmRoundTrip) {
    ...
    const auto dec = run_scli(scli(),
        std::string("aead --algo aes256-gcm --op decrypt --key ") + kKey +
...
TEST_F(AeadTests, AesGcmDecryptFailsWithWrongKey) {
    ...
    const auto dec = run_scli(scli(),
        std::string("aead --algo aes256-gcm --op decrypt --key ") + other_key +
        " --input base64:" + enc.stdout_text);
...
TEST_F(AeadTests, AesGcmRoundTripWithAad) {
    ...
    const auto enc = run_scli(scli(),
        std::string("aead --algo aes256-gcm --op encrypt --key ") + kKey +
...
TEST_F(AeadTests, ChaCha20RoundTrip) {
    const auto enc = run_scli(scli(),
        std::string("aead --algo chacha20-poly1305 --op encrypt --key ") + kKey +
        " --input " + kPlain);
    ASSERT_EQ(enc.exit_code, 0);

    const auto dec = run_scli(scli(),
        std::string("aead --algo chacha20-poly1305 --op decrypt --key ") + kKey +
        " --input base64:" + enc.stdout_text);
    EXPECT_EQ(dec.exit_code, 0);
    EXPECT_EQ(dec.stdout_text, "aGVsbG8gd29ybGQ=");
}
...
TEST_F(AeadTests, UnknownAlgoExitsNonZero) {
    const auto r = run_scli(scli(),
        std::string("aead --algo blowfish --op encrypt --key ") + kKey + " --input " + kPlain);
...
TEST_F(AeadTests, WrongKeySizeExitsNonZero) {
    ...
    const auto r = run_scli(scli(),
        std::string("aead --algo aes256-gcm --op encrypt --key ") + short_key + " --input " + kPlain);
    EXPECT_NE(r.exit_code, 0);
}
