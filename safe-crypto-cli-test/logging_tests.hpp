...
protected:
    ...
    // A valid digest invocation — succeeds cleanly, good for probing log output.
...
TEST_F(LoggingTests, EnvVarDebugEmitsTrace) {
    // Prepend the env assignment to the command run by cli_test_runner.
    // cli_test_runner on POSIX uses execv, which inherits the parent env,
    // so we set it in the parent process for this test only.
...
TEST_F(LoggingTests, WarnLevelEmitsFatalErrorOnce) {
    ...
    // Count occurrences of "nope" in stderr — should appear exactly once.
    std::size_t count = 0;
    std::size_t pos   = 0;
