...
inline void register_slh_dsa(CLI::App& app)
{
    auto* sub = app.add_subcommand("slh-dsa", "SLH-DSA (FIPS 205) signature scheme");
    sub->require_subcommand(1);

#ifndef SAFE_CRYPTO_PROVIDER_OPENSSL
    for (const char* name : {"keygen", "sign", "verify"}) {
        sub->add_subcommand(name, "")->allow_extras()->callback([]() {
            die("slh-dsa is not available in this build (requires -DSAFE_CRYPTO_ACTIVE_PROVIDER=OPENSSL)");
        });
    }
    return;
#else
    ...
    // --- keygen ---
    auto* keygen          = sub->add_subcommand("keygen", "Generate an SLH-DSA key pair");
    auto* keygen_variant  = keygen->add_option("--variant",     "Variant: sha2-128s|sha2-128f|sha2-192s|sha2-192f|sha2-256s|sha2-256f")->required();
    ...
    keygen->callback([=]() {
        ...
        else if (variant_str == "sha2-128f") { run.template operator()<SlhDsaVariant::Sha2_128f>(); }
        else if (variant_str == "sha2-192s") { run.template operator()<SlhDsaVariant::Sha2_192s>(); }
        else if (variant_str == "sha2-192f") { run.template operator()<SlhDsaVariant::Sha2_192f>(); }
        else if (variant_str == "sha2-256s") { run.template operator()<SlhDsaVariant::Sha2_256s>(); }
        else if (variant_str == "sha2-256f") { run.template operator()<SlhDsaVariant::Sha2_256f>(); }
        else { die("unknown --variant '" + variant_str + "'; valid: sha2-128s sha2-128f sha2-192s sha2-192f sha2-256s sha2-256f"); }
    });

    // --- sign ---
    auto* sign         = sub->add_subcommand("sign", "Sign a message with an SLH-DSA private key");
    auto* sign_variant = sign->add_option("--variant", "Variant: sha2-128s|sha2-128f|...")->required();
    ...
    sign->callback([=]() {
        ...
        auto run = [&]<SlhDsaVariant V>() {
            ...
            auto sig = slh_dsa_sign<V>(kp, msg);
            if (!sig) { die("slh-dsa sign failed: " + sig.error().message()); }
    ...
    // --- verify ---
    auto* verify           = sub->add_subcommand("verify", "Verify an SLH-DSA signature (exits 0=valid, 1=invalid)");
    auto* verify_variant   = verify->add_option("--variant",   "Variant: sha2-128s|sha2-128f|...")->required();
    ...
    verify->callback([=]() {
        ...
        auto run = [&]<SlhDsaVariant V>() {
            SlhDsaPublicKey<V> pk{ .public_key = SecureBuffer(key_buf->size()) };
