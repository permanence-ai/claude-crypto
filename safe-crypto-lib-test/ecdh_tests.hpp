...
TEST_F(EcdhTests, P256SharedSecretAgreementIsSymmetric) {
    ...
    const auto alice_secret = ecdh_compute_shared_secret(*alice, bob->public_key_der);
    const auto bob_secret   = ecdh_compute_shared_secret(*bob,   alice->public_key_der);
...
TEST_F(EcdhTests, P256SharedSecretHasExpectedSize) {
    ...
    const auto secret = ecdh_compute_shared_secret(*alice, bob->public_key_der);
...
TEST_F(EcdhTests, DifferentPeersProduceDifferentSecrets) {
    ...
    const auto secret_ab = ecdh_compute_shared_secret(*alice, bob->public_key_der);
    const auto secret_ac = ecdh_compute_shared_secret(*alice, carol->public_key_der);
...
TEST_F(EcdhTests, InvalidPeerPublicKeyReturnsError) {
    ...
    const auto result  = ecdh_compute_shared_secret(*alice, garbage);
