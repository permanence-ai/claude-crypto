...
// ML-DSA (FIPS 204) tests — OpenSSL provider only.
...
TEST_F(MlDsaTests, Dsa44_KeygenProducesCorrectSizes) {
    const auto kp = ml_dsa_generate_key_impl<MlDsaVariant::Dsa44, OpenSslBackend>();
...
TEST_F(MlDsaTests, Dsa44_SignVerifyRoundTrip) {
    auto kp = ml_dsa_generate_key_impl<MlDsaVariant::Dsa44, OpenSslBackend>();
    ASSERT_TRUE(kp.has_value());

    const auto msg = make_random_secure_buffer(kMessageSize);
    const auto sig = ml_dsa_sign_impl<MlDsaVariant::Dsa44, OpenSslBackend>(*kp, msg);
    ...
    const auto verify_r = ml_dsa_verify_impl<MlDsaVariant::Dsa44, OpenSslBackend>(pub, msg, *sig);
...
TEST_F(MlDsaTests, Dsa44_VerifyRejectsTamperedSignature) {
    ...
    const auto msg = make_random_secure_buffer(kMessageSize);
    auto sig = ml_dsa_sign_impl<MlDsaVariant::Dsa44, OpenSslBackend>(*kp, msg);
    ASSERT_TRUE(sig.has_value());
    ...
    const auto result = ml_dsa_verify_impl<MlDsaVariant::Dsa44, OpenSslBackend>(pub, msg, *sig);
...
TEST_F(MlDsaTests, Dsa44_VerifyRejectsTamperedMessage) {
    ...
    const auto result = ml_dsa_verify_impl<MlDsaVariant::Dsa44, OpenSslBackend>(pub, tampered, *sig);
    EXPECT_FALSE(result.has_value());
}
...
TEST_F(MlDsaTests, Dsa44_ExportImportRoundTrip) {
    ...
    const auto msg = make_random_secure_buffer(kMessageSize);
    const auto sig = ml_dsa_sign_impl<MlDsaVariant::Dsa44, OpenSslBackend>(kp2, msg);
    ASSERT_TRUE(sig.has_value());
...
TEST_F(MlDsaTests, Dsa65_SignVerifyRoundTrip) {
    auto kp = ml_dsa_generate_key_impl<MlDsaVariant::Dsa65, OpenSslBackend>();
    ASSERT_TRUE(kp.has_value());
    ...
    const auto msg = make_random_secure_buffer(kMessageSize);
    const auto sig = ml_dsa_sign_impl<MlDsaVariant::Dsa65, OpenSslBackend>(*kp, msg);
    ASSERT_TRUE(sig.has_value());
    ...
    const auto verify_r = ml_dsa_verify_impl<MlDsaVariant::Dsa65, OpenSslBackend>(pub, msg, *sig);
    EXPECT_TRUE(verify_r.has_value());
}
...
TEST_F(MlDsaTests, Dsa87_SignVerifyRoundTrip) {
    auto kp = ml_dsa_generate_key_impl<MlDsaVariant::Dsa87, OpenSslBackend>();
    ASSERT_TRUE(kp.has_value());
    ...
    const auto msg = make_random_secure_buffer(kMessageSize);
    const auto sig = ml_dsa_sign_impl<MlDsaVariant::Dsa87, OpenSslBackend>(*kp, msg);
    ASSERT_TRUE(sig.has_value());
    ...
    const auto verify_r = ml_dsa_verify_impl<MlDsaVariant::Dsa87, OpenSslBackend>(pub, msg, *sig);
    EXPECT_TRUE(verify_r.has_value());
}
