...
struct ArmAsmBackend {
        ...
        -> std::expected<SecureBuffer, Status>
    {
        ...
        const auto kv = arm_asm::detail::key_store_get(id);
        if (!kv) { return std::unexpected(err_invalid_arg); }
        SecureBuffer out(kv->len);
        std::memcpy(out.data(), kv->data.data(), kv->len);
        return out;
    ...
    static auto aead_encrypt(  // NOLINT(readability-function-size)
        ...
        -> std::expected<SecureBuffer, Status>
    {
        const auto kv_enc = arm_asm::detail::key_store_get(id);
    ...
    static auto aead_decrypt(  // NOLINT(readability-function-size)
        ...
        -> std::expected<SecureBuffer, Status>
    {
        const auto kv_dec = arm_asm::detail::key_store_get(id);
        ...
        -> std::expected<SecureBuffer, Status>
    {
        ...
        SecureBuffer out(rsa_oaep_encrypt_output_size(kv_enc->bits));
        std::size_t out_len = 0;
        ...
        -> std::expected<SecureBuffer, Status>
    {
        ...
        SecureBuffer out(rsa_oaep_decrypt_output_size(kv_dec->bits));
        std::size_t out_len = 0;
