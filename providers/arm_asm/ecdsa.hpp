    ...
    ByteSpan<p256_sig_bytes> sig_out) noexcept
{
    ...
    const Fe k = p256_scalar_from_bytes32(CByteSpan<p256_scalar_bytes>{k_buf.data(), p256_scalar_bytes});
    if (p256_scalar_is_zero(k)) { return false; }

    // R = k·G, r = R.x mod n.
    const Point R = p256_to_affine(p256_scalar_mul_base(CByteSpan<p256_scalar_bytes>{k_buf.data(), p256_scalar_bytes}));
    ...
    fe256_to_bytes(R.X, ByteSpan<p256_scalar_bytes>{rx_bytes.data(), p256_scalar_bytes});
    const Fe r = p256_scalar_from_bytes32(CByteSpan<p256_scalar_bytes>{rx_bytes.data(), p256_scalar_bytes});
    ...
    CByteSpan<p256_sig_bytes> sig) noexcept
{
    ...
    Fe r{};  // NOLINT(readability-isolate-declaration)
    Fe s{};
    ...
    ByteArray<qlen> u1b{};
    ByteArray<qlen> u2b{};
    fe256_to_bytes(u1, ByteSpan<p256_scalar_bytes>{u1b.data(), p256_scalar_bytes});
    fe256_to_bytes(u2, ByteSpan<p256_scalar_bytes>{u2b.data(), p256_scalar_bytes});

    // Compute u1·G + u2·Q.
    const Fe Qx = fe256_from_bytes(CByteSpan<p256_scalar_bytes>{public_key_uncompressed.data() + 1,  p256_scalar_bytes});
    const Fe Qy = fe256_from_bytes(CByteSpan<p256_scalar_bytes>{public_key_uncompressed.data() + 33, p256_scalar_bytes});
    ...
    fe256_to_bytes(X.X, ByteSpan<p256_scalar_bytes>{xx_bytes.data(), p256_scalar_bytes});
    const Fe xr = p256_scalar_from_bytes32(CByteSpan<p256_scalar_bytes>{xx_bytes.data(), p256_scalar_bytes});
    ...
    ByteSpan<p384_sig_bytes> sig_out) noexcept
{
    ...
    const Fe k = p384_scalar_from_bytes48(CByteSpan<p384_scalar_bytes>{k_buf.data(), p384_scalar_bytes});
    if (p384_scalar_is_zero(k)) { return false; }

    const Point R = p384_to_affine(p384_scalar_mul_base(CByteSpan<p384_scalar_bytes>{k_buf.data(), p384_scalar_bytes}));
    if (p384_point_is_identity(R)) { return false; }

    ByteArray<qlen> rx_bytes{};
    fe384_to_bytes(R.X, ByteSpan<p384_scalar_bytes>{rx_bytes.data(), p384_scalar_bytes});
    const Fe r = p384_scalar_from_bytes48(CByteSpan<p384_scalar_bytes>{rx_bytes.data(), p384_scalar_bytes});
    ...
    CByteSpan<p384_sig_bytes> sig) noexcept
{
    ...
    ByteArray<qlen> u1b{};
    ByteArray<qlen> u2b{};
    fe384_to_bytes(u1, ByteSpan<p384_scalar_bytes>{u1b.data(), p384_scalar_bytes});
    fe384_to_bytes(u2, ByteSpan<p384_scalar_bytes>{u2b.data(), p384_scalar_bytes});

    const Fe Qx = fe384_from_bytes(CByteSpan<p384_scalar_bytes>{public_key_uncompressed.data() + 1,  p384_scalar_bytes});
    const Fe Qy = fe384_from_bytes(CByteSpan<p384_scalar_bytes>{public_key_uncompressed.data() + 49, p384_scalar_bytes});
    ...
    ByteArray<qlen> xx_bytes{};
    fe384_to_bytes(X.X, ByteSpan<p384_scalar_bytes>{xx_bytes.data(), p384_scalar_bytes});
    const Fe xr = p384_scalar_from_bytes48(CByteSpan<p384_scalar_bytes>{xx_bytes.data(), p384_scalar_bytes});
    ...
    ByteSpan<p521_sig_bytes> sig_out) noexcept
{
    ...
    const Fe k = p521_scalar_from_bytes66(CByteSpan<p521_scalar_bytes>{k_buf.data(), p521_scalar_bytes});
    if (p521_scalar_is_zero(k)) { return false; }

    const Point R = p521_to_affine(p521_scalar_mul_base(CByteSpan<p521_scalar_bytes>{k_buf.data(), p521_scalar_bytes}));
    if (p521_point_is_identity(R)) { return false; }

    ByteArray<qlen> rx_bytes{};
    fe521_to_bytes(R.X, ByteSpan<p521_scalar_bytes>{rx_bytes.data(), p521_scalar_bytes});
    const Fe r = p521_scalar_from_bytes66(CByteSpan<p521_scalar_bytes>{rx_bytes.data(), p521_scalar_bytes});
    ...
    CByteSpan<p521_sig_bytes> sig) noexcept
{
    ...
    ByteArray<qlen> u1b{};
    ByteArray<qlen> u2b{};
    fe521_to_bytes(u1, ByteSpan<p521_scalar_bytes>{u1b.data(), p521_scalar_bytes});
    fe521_to_bytes(u2, ByteSpan<p521_scalar_bytes>{u2b.data(), p521_scalar_bytes});

    // Reject non-canonical P-521 encodings: top 7 bits of each coordinate's first byte must be zero.
    // public_key_uncompressed = 0x04 || x(66 bytes) || y(66 bytes): x starts at [1], y at [1+66=67].
    constexpr std::size_t p521_y_offset = 1U + p521_scalar_bytes;  // offset of y coordinate in uncompressed key
    if ((public_key_uncompressed[1]              & der_msb_flag) != 0U) { return false; } // NOLINT(cppcoreguidelines-pro-bounds-constant-array-index)
    if ((public_key_uncompressed[p521_y_offset]  & der_msb_flag) != 0U) { return false; } // NOLINT(cppcoreguidelines-pro-bounds-constant-array-index)
    const Fe Qx = fe521_from_bytes(CByteSpan<p521_scalar_bytes>{public_key_uncompressed.data() + 1,             p521_scalar_bytes});
    const Fe Qy = fe521_from_bytes(CByteSpan<p521_scalar_bytes>{public_key_uncompressed.data() + p521_y_offset, p521_scalar_bytes});
    ...
    ByteArray<qlen> xx_bytes{};
    fe521_to_bytes(X.X, ByteSpan<p521_scalar_bytes>{xx_bytes.data(), p521_scalar_bytes});
    const Fe xr = p521_scalar_from_bytes66(CByteSpan<p521_scalar_bytes>{xx_bytes.data(), p521_scalar_bytes});
