    ...
    std::span<const uint32_t, 24> c) noexcept -> Fe384
{
    ...
    r[0]  = c[0]  + c12 + c20 + (Wm1*c22) + (Wm1*c23);
    r[1]  = c[1]  + (Wm1*c12) + c13 + (Wm1*c20) + c21 + c22;
    r[2]  = c[2]  + (Wm1*c12) + (Wm1*c13) + c14 + (Wm1*c20) + (Wm1*c21) + c23;
    r[3]  = c[3]  + (Wm1*c13) + (Wm1*c14) + c15 + (Wm1*c21) + (Wm2*c22) + (Wm1*c23);
    r[4]  = c[4]  + c12 + (Wm1*c14) + (Wm1*c15) + c16 + c20 + (Wm1*c22) + (Wm3*c23);
    r[5]  = c[5]  + c13 + (Wm1*c15) + (Wm1*c16) + c17 + c21 + c22;
    r[6]  = c[6]  + c14 + (Wm1*c16) + (Wm1*c17) + c18 + c22 + (static_cast<u128>(2)*c23);
    r[7]  = c[7]  + c15 + (Wm1*c17) + (Wm1*c18) + c19 + c23;
    r[8]  = c[8]  + c16 + (Wm1*c18) + (Wm1*c19) + c20;
    r[9]  = c[9]  + c17 + (Wm1*c19) + (Wm1*c20) + c21;
    r[10] = c[10] + c18 + (Wm1*c20) + (Wm1*c21) + c22;
    r[11] = c[11] + c19 + (Wm1*c21) + (Wm1*c22) + c23;
    ...
    for (int pass = 0; pass < 2; ++pass) {
        ...
        s[4] += ov;
        for (int i = 0; i < 11; ++i) {
            s[i + 1] += s[i] >> 32; // NOLINT(hicpp-signed-bitwise)
            s[i] &= 0xffffffffLL; // NOLINT(hicpp-signed-bitwise)
        }
        s[12] = s[11] >> 32; // NOLINT(hicpp-signed-bitwise)
        s[11] &= 0xffffffffLL; // NOLINT(hicpp-signed-bitwise)
    }
...
static inline auto fe384_invert(const Fe384& a) noexcept -> Fe384 {
    ...
    t = p4;
    for (int i = 0; i < 4; ++i) { t = fe384_sqr(t); }
    const Fe384 p8 = fe384_mul(t, p4);                     // a^(2^8-1)
    t = p8;
    for (int i = 0; i < 8; ++i) { t = fe384_sqr(t); }
    const Fe384 p16 = fe384_mul(t, p8);                    // a^(2^16-1)
    t = p16;
    for (int i = 0; i < 16; ++i) { t = fe384_sqr(t); }
    ...
    for (int blk = 0; blk < 6; ++blk) {
        for (int i = 0; i < 32; ++i) { acc = fe384_sqr(acc); }
    ...
    // Bits 95..32: 64 zeros.
    for (int i = 0; i < 64; ++i) { acc = fe384_sqr(acc); }
