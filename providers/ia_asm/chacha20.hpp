                            ...
                            const CryptoByte* in, CryptoByte* out, std::size_t len) noexcept // NOLINT(readability-non-const-parameter)
{
    ...
    while (len - offset >= 64U) {
        chacha20_block(key, counter, nonce, ByteSpan<chacha20_block_bytes>{block.data(), chacha20_block_bytes});
    ...
    if (offset < len) {
        ...
        for (std::size_t i = 0; offset + i < len; ++i) {
            ...
            out[offset + i] = static_cast<CryptoByte>(in[offset + i] ^ block[i]);
        }
    }
}
...
{
    FixedSecureBuffer<chacha20_block_bytes> block;
    chacha20_block(key, 0, nonce, ByteSpan<chacha20_block_bytes>{block.data(), chacha20_block_bytes});
