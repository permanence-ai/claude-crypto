...
struct IaAsmBackend {
        ...
        -> std::expected<SecureBuffer, Status>
    {
        if (alg == alg_sha(ShaVariant::Sha256)) {
            ...
            ia_asm::detail::sha256(input.data(), input.size(), ByteSpan<sha256_digest_bytes>{output.data(), sha256_digest_bytes});
            return output;
        }
        if (alg == alg_sha(ShaVariant::Sha512)) {
            SecureBuffer output(sha512_size_bytes);
            ia_asm::detail::sha512(input.data(), input.size(), ByteSpan<sha512_digest_bytes>{output.data(), sha512_digest_bytes});
            return output;
        }
        if (alg == alg_sha(ShaVariant::Sha384)) {
            SecureBuffer output(sha384_size_bytes);
            ia_asm::detail::sha384(input.data(), input.size(), ByteSpan<sha384_digest_bytes>{output.data(), sha384_digest_bytes});
            return output;
        }
        if (alg == alg_sha(ShaVariant::Sha3_256)) {
            SecureBuffer output(sha3_256_size_bytes);
            ia_asm::detail::sha3_256(input.data(), input.size(), ByteSpan<sha3_256_digest_bytes>{output.data(), sha3_256_digest_bytes});
            return output;
        }
        if (alg == alg_sha(ShaVariant::Sha3_384)) {
            SecureBuffer output(sha3_384_size_bytes);
            ia_asm::detail::sha3_384(input.data(), input.size(), ByteSpan<sha3_384_digest_bytes>{output.data(), sha3_384_digest_bytes});
            return output;
        }
        if (alg == alg_sha(ShaVariant::Sha3_512)) {
            SecureBuffer output(sha3_512_size_bytes);
            ia_asm::detail::sha3_512(input.data(), input.size(), ByteSpan<sha3_512_digest_bytes>{output.data(), sha3_512_digest_bytes});
            return output;
        }
    ...
    [[nodiscard]]
    static auto import_key(const KeyAttributes* attrs, const CByteVSpan data) // NOLINT(readability-function-size,readability-function-cognitive-complexity)
        ...
        if (attrs != nullptr && attrs->ec_curve != arm_asm::detail::EcCurveId::None) {
            const KeyId slot = arm_asm::detail::ec_key_store_import(
                attrs->ec_curve, attrs->ec_kind, data.data(), data.size());
        ...
        if (attrs != nullptr && attrs->rsa_key_kind != arm_asm::detail::RsaKeyKind::None) {
            const KeyId slot = arm_asm::detail::rsa_key_store_import(
                attrs->rsa_key_kind, attrs->rsa_bits, data.data(), data.size());
            if (slot == 0U) { return std::unexpected(err_invalid_arg); }
            return slot;
        }
        ...
        const KeyId slot = ia_asm::detail::key_store_import(data.data(), data.size());
        if (slot == 0U) { return std::unexpected(err_invalid_arg); }
        return slot;
    }
        ...
        -> std::expected<SecureBuffer, Status>
    {
        ...
        if (alg == alg_aes_gcm()) {
            ...
            SecureBuffer out(pt.size() + ia_asm::detail::aes_gcm_tag_bytes);
            ia_asm::detail::aes256_gcm_encrypt(key, nonce.data(), aad.data(), aad.size(), pt.data(), pt.size(), out.data());
            return out;
        }
        if (alg == alg_chacha20_poly1305()) {
            if (nonce.size() != ia_asm::detail::chacha20_poly1305_nonce_bytes) { return std::unexpected(err_invalid_arg); }
            if (pt.size() > SIZE_MAX - ia_asm::detail::chacha20_poly1305_tag_bytes) { return std::unexpected(err_invalid_arg); }
            SecureBuffer out(pt.size() + ia_asm::detail::chacha20_poly1305_tag_bytes);
            ia_asm::detail::chacha20_poly1305_encrypt(key, nonce.data(), aad.data(), aad.size(), pt.data(), pt.size(), out.data());
            return out;
        }
        return std::unexpected(err_invalid_arg);
    }
        ...
        -> std::expected<SecureBuffer, Status>
    {
        ...
        if (alg == alg_aes_gcm()) {
            ...
            const std::size_t pt_len = ct.size() - ia_asm::detail::aes_gcm_tag_bytes;
            SecureBuffer out(ct.size());
            if (!ia_asm::detail::aes256_gcm_decrypt(key, nonce.data(), aad.data(), aad.size(), ct.data(), ct.size(), out.data())) {
        ...
        if (alg == alg_chacha20_poly1305()) {
            if (nonce.size() != ia_asm::detail::chacha20_poly1305_nonce_bytes) { return std::unexpected(err_invalid_arg); }
            if (ct.size() < ia_asm::detail::chacha20_poly1305_tag_bytes) { return std::unexpected(err_invalid_arg); }
            const std::size_t pt_len = ct.size() - ia_asm::detail::chacha20_poly1305_tag_bytes;
            SecureBuffer out(ct.size());
            if (!ia_asm::detail::chacha20_poly1305_decrypt(key, nonce.data(), aad.data(), aad.size(), ct.data(), ct.size(), out.data())) {
                return std::unexpected(err_invalid_sig);
            }
            out.resize(pt_len);
            return out;
    ...
    static Status verify_message(KeyId id, Algorithm alg, // NOLINT(readability-function-size,readability-function-cognitive-complexity,bugprone-easily-swappable-parameters)
        ...
        if (curve == EcCurveId::P256) {
            ...
            if (sig.size() != expected_sig_len || key_len != pk_len) { return err_invalid_arg; }
            ByteArray<hash_len> hash{};
            ia_asm::detail::sha256(msg.data(), msg.size(), hash);
        ...
        if (curve == EcCurveId::P384) {
            ...
            if (sig.size() != expected_sig_len || key_len != pk_len) { return err_invalid_arg; }
            ByteArray<hash_len> hash{};
            ia_asm::detail::sha384(msg.data(), msg.size(), hash);
        ...
        if (curve == EcCurveId::P521) {
            ...
            if (sig.size() != expected_sig_len || key_len != pk_len) { return err_invalid_arg; }
            ByteArray<hash_len> hash{};
            ia_asm::detail::sha512(msg.data(), msg.size(), hash);
