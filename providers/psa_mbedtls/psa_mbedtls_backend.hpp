...
struct RealPsaBackend {
    ...
    // ML-KEM — not supported by MbedTLS 4.1; all operations return err_invalid_arg.
    [[nodiscard]]
    static Algorithm alg_ml_kem(const MlKemVariant) noexcept {
        return static_cast<Algorithm>(0);  // PSA_ALG_NONE — unsupported
    }
    [[nodiscard]]
    static KeyAttributes make_ml_kem_generate_attrs(const MlKemVariant) noexcept {
        const KeyAttributes a = PSA_KEY_ATTRIBUTES_INIT;
        return a;
    }
    [[nodiscard]]
    static KeyAttributes make_ml_kem_encap_attrs(const MlKemVariant) noexcept {
        const KeyAttributes a = PSA_KEY_ATTRIBUTES_INIT;
        return a;
    }
    [[nodiscard]]
    static KeyAttributes make_ml_kem_decap_attrs(const MlKemVariant) noexcept {
        const KeyAttributes a = PSA_KEY_ATTRIBUTES_INIT;
        return a;
    }
    [[nodiscard]]
    ...
    static Status kem_encapsulate(
        ...
        CryptoByte*, std::size_t, std::size_t*) noexcept {
